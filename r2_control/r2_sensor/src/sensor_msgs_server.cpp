...
bool doReq(r2_msgs::action_cmd::Request& req,r2_msgs::action_cmd::Response& resp)
{
    ...
    if(if_use_gazebo==0 && field_direction==RIGHT)
    {
        resp.POS_X.data = req.odomData.pose.pose.position.x*1000 + 5500;
        resp.POS_Y.data = req.odomData.pose.pose.position.y*1000 + 500;
    }
    else if(if_use_gazebo==0 && field_direction==LEFT)
    {
        resp.POS_X.data = req.odomData.pose.pose.position.x*1000 - 5500;
        resp.POS_Y.data = req.odomData.pose.pose.position.y*1000 + 500;
    }
    ...
    if(if_use_gazebo==1)
    {
        resp.POS_X.data = req.odomData.pose.pose.position.x*1000;
        resp.POS_Y.data = req.odomData.pose.pose.position.y*1000;
    }
...
int main(int argc, char *argv[])
{
    ...
    ros::NodeHandle nh;
    ros::ServiceServer server = nh.advertiseService("/action_cmd", doReq);
    std::string direction;

    if(!nh.param("if_use_gazebo",if_use_gazebo,1))
    {
