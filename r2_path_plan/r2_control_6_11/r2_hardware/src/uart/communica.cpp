...
namespace control_base
{
    ...
    void CONTROL_BASE::UPPER_TO_STM32(unsigned char next_ctrl_state,unsigned char take_ball_flag)
    {
        // 协议数据缓存数组
        ...
        // 控制指令
        Upper_Buf[3 + Length - 2] = next_ctrl_state;                  //Buf[3]
        Upper_Buf[3 + Length - 1] = take_ball_flag;                  //Buf[4]
       
        
        // 设置校验值、消息尾
        Upper_Buf[3 + Length] = serial_get_crc8_value(Upper_Buf, 3 + Length);	//buf[5]
        Upper_Buf[3 + Length + 1] = serial_ender[0];                      //buf[6]
        Upper_Buf[3 + Length + 2] = serial_ender[1];                      //buf[7]
