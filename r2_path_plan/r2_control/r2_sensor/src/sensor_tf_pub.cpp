...
void doPose(const nav_msgs::Odometry::ConstPtr& pose){
    ...
    if(if_use_gazebo==1)
    {
        coordinate.x = pose->pose.pose.position.x*1000;
        coordinate.y = pose->pose.pose.position.y*1000;
...
int main(int argc, char *argv[])
{
    ...
    ros::Subscriber sub = nh.subscribe<nav_msgs::Odometry>("/odom",1,doPose);
    odom_client_ = nh.serviceClient<r2_msgs::action_cmd>("/action_cmd");
